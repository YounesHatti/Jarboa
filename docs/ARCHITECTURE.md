...
Jarboa uses Smack's Signal-backed legacy OMEMO namespace for interoperability with Conversations-family clients. It follows Smack's documented lifecycle: install one persistent store for the application process, prepare the manager and listeners before login, then initialize and publish the local bundle after authentication. Jarboa separately verifies that the device-list and per-device bundle PEP nodes have open read access, as required for encrypted sessions to begin before contacts have presence subscriptions. A server-side node-configuration error is reported as an interoperability warning instead of invalidating a successfully initialized local cryptographic session. New direct-message bodies are encrypted to the contact's active devices and the sender's other known devices. Sending fails closed when there are no compatible recipient devices, a fingerprint changed, or an accepted device could not be encrypted. Incoming plaintext is stored only with an explicit unencrypted label.
